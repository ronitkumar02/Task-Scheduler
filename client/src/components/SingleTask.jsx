    ...
    return (
        <div className="rounded-lg shadow-xl px-12 py-8 flex flex-col mx-20 relative w-2/3 justify-self-center bg-gray-100">
            <div className='flex flex-row items-center'>
                ...
                <AiOutlineCheck className='text-3xl ml-2 font-bold text-sky-300' />
            </div>
            <p className="text-lg my-4">{task.description}</p>
            <div className='flex items-center justify-between'>
                <div className='flex items-center'>
                    <span className='mr-2 font-bold'>Priority:</span>
                    <Rating value={task.priority} color={'#4973a1'} />
                </div>
                <Link to={`/tasks/${task._id}`} className='bg-teal-600 text-white rounded-md px-4 py-2 font-bold hover:opacity-80 transition duration-200'>View More</Link>
            </div>
